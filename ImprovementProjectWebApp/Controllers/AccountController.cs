...
namespace ImprovementProjectWebApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        //add role Manager in the accountController 
        //there is 3 place need to add ***
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!x)
                    {
                        ...
                        superUser.UserName = "dx3081@gmail.com";
                        superUser.Email = "dx3081@gmail.com";

                        string userPWD = "Dx3081cctv!";

                        IdentityResult chkUser = await _userManager.CreateAsync(superUser, userPWD);

                        //Add default User to Role SuperAdmin    
                        if (chkUser.Succeeded)
                        {
                            var result1 = await _userManager.AddToRoleAsync(superUser, "SuperAdmin");
                            //so this line of code is the core of everything.
                            //it assign the user to the role. 
                        }

                        //or we can just use first register being superadmin. but i feel like it is not safe ....what if someone register before me = =b
                    ...
                    // creating Creating LookUpUser role    
                    x = await _roleManager.RoleExistsAsync("LookUpUser");
                    if (!x)
                    {
                        var role = new IdentityRole();
                        role.Name = "LookUpUser";
                        await _roleManager.CreateAsync(role);
                    }



                    //=============regular user assign role =================
